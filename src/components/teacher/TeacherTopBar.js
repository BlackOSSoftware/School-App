...
function TeacherTopBar({ title, onBack, onNotificationPress }) {
    ...
    <View style={[styles.topBar, { paddingTop: Math.max(insets.top, 8) + 6 }]}>
      <View style={styles.leftWrap}>
        {onBack ? (
          <Pressable style={styles.backBtn} onPress={onBack}>
            <AppIcon name="chevron-back" size={20} color={colors.teacher.accent} />
          </Pressable>
        ) : null}
        <View>
          <Text style={styles.kicker}>MMPS TEACHER PANEL</Text>
      ...
      <View style={styles.rightWrap}>
        ...
        <Pressable style={styles.iconBtn} onPress={onNotificationPress}>
          <AppIcon name="notifications-outline" size={18} color={colors.teacher.accent} />
        </Pressable>
      </View>
    </View>
  );
...
const createStyles = colors =>
  StyleSheet.create({
    topBar: {
      ...
      borderBottomWidth: 1,
      borderBottomColor: colors.teacher.borderSoft,
    },
    leftWrap: {
      flexDirection: 'row',
      alignItems: 'center',
