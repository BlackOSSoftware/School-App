...
const HomeworkListItem = memo(function HomeworkListItem({ item, styles, onPress, brandColor }) {
  return (
...
export default function StudentHomeworkScreen({ prefillSelectedItem = null }) {
  ...
  const renderItem = useCallback(
    ({ item }) => <HomeworkListItem item={item} styles={styles} onPress={openDetails} brandColor={colors.brand.primary} />,
  ...
  const listFooter = useMemo(
    () => (
      <>
        {!rows.length && !query.isLoading ? <Text style={styles.emptyText}>No homework found.</Text> : null}
